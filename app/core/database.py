...
def init_metadata_tables():
    """Create internal metadata tables for system prompts and guardrails."""
    ...
    try:
        ...
        # Insert default analysis type if none exists
...
def get_all_tables() -> list[dict]:
    ...
    internal = {"analysis_types", "api_keys", "query_history", "sqlite_sequence"}
    conn = get_sync_connection()
    try:
        cursor = conn.execute(
...
def get_table_schema_text() -> str:
    """Return a textual description of all user tables for the LLM context."""
